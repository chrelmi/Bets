...
squadracs="Bologna" #input("\nInserisci la squadra di casa:  ")
squadratr="Chievo" #input("Inserisci la squadra in trasferta:  ")

#picchetto tecnico
pf=pf-5
with open('risultati.csv') as csvfile:
...
pfcasa=round(((cvpf+tppf)/5)*100,2)
picchettotr=(pc+vt)/(cs+tr)
picchettotr=picchettotr*100
pftrasferta=round(((cppf+tvpf)/5)*100,2)
pfx=100-(pfcasa-pftrasferta)
...
quotax=str(quotax)
qpfc=round(100/pfcasa,2)
qpft=round(100/pftrasferta,2)
qpfx=round(100/pfx,2)
qpfc=str(qpfc)
qpft=str(qpft)
qpfx=str(qpfx)
#stampe
